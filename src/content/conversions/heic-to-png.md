...
limitations:
  - The output is genuinely large. Converting a folder of iPhone photos to PNG
    needs meaningfully more disk than the originals did, so check the
    destination has room before starting a big batch.
...
barely compress at all, because adjacent pixels differ by small random amounts
from sensor noise.
...
reproduces that same 8-bit result exactly, and is usually smaller. See that page
for Google's published figures and their baselines.
