...
| AVIF encode is **3–5× mozjpeg and 2.5–3.5× WebP**                                              | Three runs of the same harness — 12 MP synthetic photographic source, median of 3 encodes each, `jpeg({quality:80, mozjpeg:true})` / `webp({quality:80})` / `avif({quality:80})`. **Run 1** (sharp 0.35.4 / libvips 8.18.6, NOT the bundled build): 446 / 762 / 2308 ms → 5.2× and 3.0×. **Run 2** (bundled 0.35.3 / 8.18.3): 614 / 758 / 2464 ms → 4.0× and 3.25×. **Run 3** (bundled 0.35.3 / 8.18.3): 435 / 599 / 1496 ms → 3.4× and 2.5×. The published range spans all three, including the WebP ratios, which an earlier revision of this row omitted. Run 1 is kept for transparency but runs on a library build the app does not ship — which is itself why the site publishes a range rather than a figure. Both pages describe the method as "across repeat runs": each run was a median, and the RANGE comes from the three runs disagreeing. |
